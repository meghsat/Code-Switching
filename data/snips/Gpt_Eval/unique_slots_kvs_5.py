...
# Convert sets to lists and keep only 5 random items per slot
slot_dict = {
    k: random.sample(sorted(list(v)), min(5, len(v)))
...
print(f"\n✅ Extracted {len(slot_dict)} slot categories → saved 5 samples each to '{output_json}'\n")

# Print the selected 5 words per slot
